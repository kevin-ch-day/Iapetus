...
from iapetus.learning.deep.metrics import binary_confusion_matrix, binary_precision_recall
from iapetus.learning.deep.model import PurePythonMLP, torch_available, train_torch_mlp
...
def _train_model(
    ...
    *,
    ...
    epochs, learning_rate = _training_hyperparams(output_size, len(matrix))
...
def leave_one_out_cv(
    ...
    *,
    ...
    for holdout_index in range(len(matrix)):
        ...
        model, _ = _train_model(train_x, train_y, output_size=output_size, backend=backend, seed=seed + holdout_index)
        if backend == "torch" and torch_available():
            import torch
...
def train_static_mlp(
    *,
    ...
    if len(examples) < 4:
        raise ValueError("Training corpus too small for static-v1 deep learning (need >= 4 eligible examples).")

    matrix, entity_labels, class_labels, norm_stats, class_to_index, index_to_class = prepare_training_batch(
        examples
    )
    ...
    malware_class_model, malware_class_loss = _train_model(
        malware_matrix,
        encode_classification_labels(malware_examples, malware_class_to_index),
    ...
    benign_class_model, benign_class_loss = _train_model(
        benign_matrix,
        encode_classification_labels(benign_examples, benign_class_to_index),
