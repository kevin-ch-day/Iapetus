...
from iapetus.learning.deep.neural_network_models import PurePythonMLP, torch_available
...
def find_saved_model_run_dir(
    ...
    if not candidates:
        raise FileNotFoundError(f"No saved static MLP model runs under {output_dir}")
    return candidates[0]


def find_static_v1_run_dir(output_dir: Path, run_id: str | None = None) -> Path:
    """Backward-compatible alias for find_saved_model_run_dir."""
    return find_saved_model_run_dir(output_dir, run_id=run_id)


def load_model_bundle(output_dir: Path, run_id: str | None = None) -> ModelBundle:
    run_dir = find_saved_model_run_dir(output_dir, run_id=run_id)
...
def evaluate_saved_run(bundle: ModelBundle) -> dict[str, Any]:
    ...
    malware_examples = [example for example in examples if example["entity_kind"] == "malware"]
    benign_examples = [example for example in examples if example["entity_kind"] == "normal_app"]
    rows = [encode_row(example["feature_vector"]) for example in examples]
    matrix = apply_normalization(rows, bundle.normalization)
    predictions: list[dict[str, Any]] = []
    correct_entity = 0
    correct_class = 0
    correct_malware_class = 0
    correct_benign_class = 0
    for example, row in zip(examples, matrix, strict=True):
        ...
        if example["entity_kind"] == "malware":
            correct_malware_class += int(class_ok)
        else:
            correct_benign_class += int(class_ok)
        predictions.append(
            {
    ...
    count = len(examples)
    malware_count = len(malware_examples)
    ...
    return {
        ...
        "classification_subgroup_accuracy": {
            "malware": round(correct_malware_class / malware_count, 4) if malware_count else 0.0,
            "benign": round(correct_benign_class / benign_count, 4) if benign_count else 0.0,
        },
